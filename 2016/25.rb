...
def assembunny(regs, input)
  ...
  while current_index < input.size
    ...
    when 'out'
      next if current.size != 2
      if ['a', 'b', 'c', 'd'].include?(current[1])
        yield regs[current[1]]
      else
        yield current[1].to_i
      end
    end
...
(1..).each do |i|
  signal = []
  assembunny({'a' => i, 'b' => 0, 'c' => 0, 'd' => 0}, input) do |clock_signal|
    break unless [0, 1].include?(clock_signal)
    break if signal.last == clock_signal
    signal << clock_signal
    if signal.size > 8
      p i
      exit
